...
const getMaptilerStyle = (isDark: boolean): string | maplibregl.StyleSpecification => {
  if (MAPTILER_API_KEY) {
    // Use Maptiler Dataviz styles with API key (recommended)
    return isDark
      ? `https://api.maptiler.com/maps/dataviz-v4-dark/style.json?key=${MAPTILER_API_KEY}`
      : `https://api.maptiler.com/maps/dataviz-v4-light/style.json?key=${MAPTILER_API_KEY}`;
  } else {
...
export const Map = ({ companies, onCompanyClick, selectedCompanyId, className = '' }: MapProps) => {
  ...
  useEffect(() => {
    ...
    // Initialize map centered on Bulgaria with Maptiler Dataviz style
    map.current = new maplibregl.Map({
      container: mapContainer.current,
      style: getMaptilerStyle(isDarkMode),
  ...
  useEffect(() => {
    ...
    map.current.setStyle(getMaptilerStyle(isDarkMode));
